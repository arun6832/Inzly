...
import { Trophy, Globe, Medal, User } from "lucide-react";
...
const COUNTRIES = ["United States", "India", "United Kingdom", "Canada", "Australia", "Other", "Unknown"];
...
export default function LeaderboardPage() {
    ...
    useEffect(() => {
        const fetchOverall = async () => {
            try {
                const q = query(collection(db, "users"), orderBy("totalLikes", "desc"), limit(50));
                const snap = await getDocs(q);
                const users: LeaderboardUser[] = [];
                snap.forEach(doc => {
                    const data = doc.data();
                    users.push({ id: doc.id, name: data.name, totalLikes: data.totalLikes || 0, country: data.country || "Unknown" });
                });
    ...
    useEffect(() => {
        const fetchCountry = async () => {
            ...
            try {
                ...
                snap.forEach(doc => {
                    const data = doc.data();
                    users.push({ id: doc.id, name: data.name, totalLikes: data.totalLikes || 0, country: data.country });
                });
    ...
    const renderUserItem = (u: LeaderboardUser, index: number) => {
        ...
        return (
            <motion.div 
                ...
                key={u.id} 
                className={`flex items-center justify-between p-4 sm:p-6 bg-[#121218] border ${user?.uid === u.id ? 'border-purple-500/50 bg-purple-500/5' : 'border-white/[0.04]'} rounded-3xl shadow-lg hover:border-white/10 transition-colors`}
            >
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-zinc-500 text-sm">{u.country}</p>
                    </div>
                </div>
                ...
                <Tabs defaultValue="overall" className="w-full">
                    ...
                    <TabsContent value="overall" className="space-y-4 animate-in fade-in-50 duration-500">
                        {loadingOverall ? (
                            ...
                            <div className="space-y-3">
                                {overallUsers.map((u, i) => renderUserItem(u, i))}
                            </div>
                        )}
                    </TabsContent>
                    ...
                    <TabsContent value="country" className="space-y-6 animate-in fade-in-50 duration-500">
                        ...
                        {loadingCountry ? (
                            ...
                            <div className="text-center py-20 text-zinc-500 bg-[#121218] rounded-[32px] border border-white/[0.04]">
                                No top users found in {selectedCountry} yet.
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {countryUsers.map((u, i) => renderUserItem(u, i))}
                            </div>
                        )}
                    </TabsContent>
                </Tabs>
